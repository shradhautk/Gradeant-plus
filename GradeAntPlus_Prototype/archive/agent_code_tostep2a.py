...
def human_interaction_tool(tool_context: ToolContext, prompt: str) -> dict:
    ...
    print("\n💬 CONVERSATION STATE:")
    history = tool_context.session.state.get("conversation_history", [])
    ...
    try:
        ...
        logger.info("User interrupted input, treating as exit request")
        human_input = "exit"
    
    logger.info(f"HumanTool: Received input: '{human_input}'")
    
    # Allow the user to exit the loop early
    if human_input.lower() in ['done', 'exit', 'got it', 'thanks', 'quit', '']:
        ...
        tool_context.actions.escalate = True  # This signals the parent LoopAgent to stop
    ...
    tool_context.session.state["conversation_history"] = history
...
async def grade_ant_plus_main():
    ...
    for idx, question_data in enumerate(input_questions, 1):
        ...
        async for event in runner.run_async(
            ...
            # Check for function calls in the event content
            if event.content and hasattr(event.content, 'parts'):
                for part in event.content.parts:
                    if hasattr(part, 'function_call') and part.function_call:
                        logger.info(f"Function call detected: {part.function_call.name}")
                        print(f"🔧 Agent {event.author} calling function: {part.function_call.name}")
                    elif hasattr(part, 'function_response') and part.function_response:
                        logger.info(f"Function response from: {part.function_response.name}")
